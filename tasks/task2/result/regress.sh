...
echo "🧪 Загрузка фикстур для сервиса booking-service..."
PGPASSWORD="${DB_PASSWORD}" psql -h "${BOOKING_SERVICE_DB_HOST}" -p "${DB_PORT}" -U "${DB_USER}" "${DB_NAME}" < init-fixtures-booking-service.sql
